...
if (isset($_POST['add_station'])) {
    ...
    if (!empty($station_name) && !empty($username) && !empty($password)) {
        // Hash password
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        try {
            $stmt = $pdo->prepare("INSERT INTO polling_stations (station_name, username, password) VALUES (:station_name, :username, :password)");
            $stmt->execute(['station_name' => $station_name, 'username' => $username, 'password' => $hashed_password]);
            $_SESSION['success'] = "Station added successfully";
        } catch (PDOException $e) {
            $_SESSION['error'] = "Error adding station: " . $e->getMessage();
        }
    } else {
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    try {
        $stmt = $pdo->prepare("DELETE FROM polling_stations WHERE id = :id");
        $stmt->execute(['id' => $id]);
...
// Fetch Stations
$stations = $pdo->query("SELECT * FROM polling_stations ORDER BY station_name ASC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Manage Polling Stations - BOA AMPONSEM SHS</title>
    ...
    <div class="wrapper">
        <nav class="sidebar">
            ...
            <ul>
                ...
                <li class="active"><a href="stations.php"><i class="fas fa-building"></i> Polling Stations</a></li>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="stations.php?delete=<?php echo $station['id']; ?>" class="btn btn-danger" onclick="return confirm('Are you sure?')">Delete</a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
